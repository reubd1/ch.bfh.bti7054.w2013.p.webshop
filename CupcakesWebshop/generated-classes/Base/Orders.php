...
abstract class Orders implements ActiveRecordInterface
{
     ...
     *
     * @return Orders The current object, for fluid interface
     */
    ...
    public function setOrderId($v)
    {
        ...
        if ($this->order_id !== $v) {
            $this->order_id = $v;
            $this->modifiedColumns[OrdersTableMap::ORDER_ID] = true;
        }
    ...
    public function setTotal($v)
    {
        ...
        if ($this->total !== $v) {
            $this->total = $v;
            $this->modifiedColumns[OrdersTableMap::TOTAL] = true;
        }
    ...
    public function setOrderDate($v)
    {
        ...
        if ($this->order_date !== null || $dt !== null) {
            if ($dt !== $this->order_date) {
                $this->order_date = $dt;
                $this->modifiedColumns[OrdersTableMap::ORDER_DATE] = true;
            }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[OrdersTableMap::USER_ID] = true;
        }
    ...
    public function setShippingId($v)
    {
        ...
        if ($this->shipping_id !== $v) {
            $this->shipping_id = $v;
            $this->modifiedColumns[OrdersTableMap::SHIPPING_ID] = true;
        }
    ...
    public function setBillingId($v)
    {
        ...
        if ($this->billing_id !== $v) {
            $this->billing_id = $v;
            $this->modifiedColumns[OrdersTableMap::BILLING_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : OrdersTableMap::translateFieldName('Total', TableMap::TYPE_PHPNAME, $indexType)];
            $this->total = (null !== $col) ? (double) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : OrdersTableMap::translateFieldName('OrderDate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrdersTableMap::translateFieldName('ShippingId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->shipping_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : OrdersTableMap::translateFieldName('BillingId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = OrdersTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Orders object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null !== $this->order_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . OrdersTableMap::ORDER_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(OrdersTableMap::ORDER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'ORDER_ID';
        }
        if ($this->isColumnModified(OrdersTableMap::TOTAL)) {
            $modifiedColumns[':p' . $index++]  = 'TOTAL';
        }
        if ($this->isColumnModified(OrdersTableMap::ORDER_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'ORDER_DATE';
        }
        if ($this->isColumnModified(OrdersTableMap::USER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'USER_ID';
        }
        if ($this->isColumnModified(OrdersTableMap::SHIPPING_ID)) {
            $modifiedColumns[':p' . $index++]  = 'SHIPPING_ID';
        }
        if ($this->isColumnModified(OrdersTableMap::BILLING_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrdersTableMap::ORDER_ID)) $criteria->add(OrdersTableMap::ORDER_ID, $this->order_id);
        if ($this->isColumnModified(OrdersTableMap::TOTAL)) $criteria->add(OrdersTableMap::TOTAL, $this->total);
        if ($this->isColumnModified(OrdersTableMap::ORDER_DATE)) $criteria->add(OrdersTableMap::ORDER_DATE, $this->order_date);
        if ($this->isColumnModified(OrdersTableMap::USER_ID)) $criteria->add(OrdersTableMap::USER_ID, $this->user_id);
        if ($this->isColumnModified(OrdersTableMap::SHIPPING_ID)) $criteria->add(OrdersTableMap::SHIPPING_ID, $this->shipping_id);
        if ($this->isColumnModified(OrdersTableMap::BILLING_ID)) $criteria->add(OrdersTableMap::BILLING_ID, $this->billing_id);
    ...
    {
        $criteria = new Criteria(OrdersTableMap::DATABASE_NAME);
        $criteria->add(OrdersTableMap::ORDER_ID, $this->order_id);
    ...
    public function getShippingAddress(ConnectionInterface $con = null)
    {
        if ($this->aShippingAddress === null && ($this->shipping_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aShippingAddress->addOrderss($this);
             */
        }
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildOrders The current object (for fluent API support)
     */
    public function setOrderItemss(Collection $orderItemss, ConnectionInterface $con = null)
    {
        ...
        foreach ($orderItemssToDelete as $orderItemsRemoved) {
            $orderItemsRemoved->setOrders(null);
        }
    ...
    public function setCustomItems(Collection $customItems, ConnectionInterface $con = null)
    {
        ...
        foreach ($customItemsToDelete as $customItemRemoved) {
            $customItemRemoved->setOrders(null);
        }
