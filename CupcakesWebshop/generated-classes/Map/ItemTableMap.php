...
class ItemTableMap extends TableMap
{
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('ItemId', 'Name', 'Price', 'Image', 'Available', 'CategoryId', ),
        self::TYPE_STUDLYPHPNAME => array('itemId', 'name', 'price', 'image', 'available', 'categoryId', ),
        self::TYPE_COLNAME       => array(ItemTableMap::ITEM_ID, ItemTableMap::NAME, ItemTableMap::PRICE, ItemTableMap::IMAGE, ItemTableMap::AVAILABLE, ItemTableMap::CATEGORY_ID, ),
        self::TYPE_RAW_COLNAME   => array('ITEM_ID', 'NAME', 'PRICE', 'IMAGE', 'AVAILABLE', 'CATEGORY_ID', ),
        self::TYPE_FIELDNAME     => array('item_id', 'name', 'price', 'image', 'available', 'category_id', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('ItemId' => 0, 'Name' => 1, 'Price' => 2, 'Image' => 3, 'Available' => 4, 'CategoryId' => 5, ),
        self::TYPE_STUDLYPHPNAME => array('itemId' => 0, 'name' => 1, 'price' => 2, 'image' => 3, 'available' => 4, 'categoryId' => 5, ),
        self::TYPE_COLNAME       => array(ItemTableMap::ITEM_ID => 0, ItemTableMap::NAME => 1, ItemTableMap::PRICE => 2, ItemTableMap::IMAGE => 3, ItemTableMap::AVAILABLE => 4, ItemTableMap::CATEGORY_ID => 5, ),
        self::TYPE_RAW_COLNAME   => array('ITEM_ID' => 0, 'NAME' => 1, 'PRICE' => 2, 'IMAGE' => 3, 'AVAILABLE' => 4, 'CATEGORY_ID' => 5, ),
        self::TYPE_FIELDNAME     => array('item_id' => 0, 'name' => 1, 'price' => 2, 'image' => 3, 'available' => 4, 'category_id' => 5, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, )
    );
    ...
    public function initialize()
    {
        ...
        $this->addForeignKey('CATEGORY_ID', 'CategoryId', 'INTEGER', 'category', 'CATEGORY_ID', true, null, null);
    } // initialize()
    ...
    {
        $this->addRelation('Category', '\\Category', RelationMap::MANY_TO_ONE, array('category_id' => 'category_id', ), null, null);
        $this->addRelation('ItemDescription', '\\ItemDescription', RelationMap::ONE_TO_MANY, array('item_id' => 'item_id', ), null, null, 'ItemDescriptions');
        $this->addRelation('OrderItems', '\\OrderItems', RelationMap::ONE_TO_MANY, array('item_id' => 'item_id', ), null, null, 'OrderItemss');
    } // buildRelations()
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(ItemTableMap::ITEM_ID);
            $criteria->addSelectColumn(ItemTableMap::NAME);
            $criteria->addSelectColumn(ItemTableMap::PRICE);
            $criteria->addSelectColumn(ItemTableMap::IMAGE);
            $criteria->addSelectColumn(ItemTableMap::AVAILABLE);
            $criteria->addSelectColumn(ItemTableMap::CATEGORY_ID);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            $criteria->add(ItemTableMap::ITEM_ID, (array) $values, Criteria::IN);
        }

        $query = ItemQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { ItemTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { ItemTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(ItemTableMap::ITEM_ID) && $criteria->keyContainsValue(ItemTableMap::ITEM_ID) ) {
